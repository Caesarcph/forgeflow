    ...
    if (stopCondition) {
      if (stopCondition.shouldStop) {
        ...
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Autopilot stopped: ${stopCondition.reason}`,
        });
        return;
      }
      if (stopCondition.requiresApproval) {
        ...
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Autopilot paused: ${stopCondition.reason}. Waiting for approval to continue.`,
        });
    ...
    if (run.result === "waiting_human") {
      ...
      if (task?.autoApprovable && !config.stopOnHumanGate && !touchesHighRisk && sensitiveFiles.length === 0) {
        ...
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Autopilot auto-approved ${task.taskCode}. Completed tasks this session: ${session.tasksCompleted}.`,
        });
...
async function runProjectSafeAutopilotLoop(projectId: string) {
  ...
  while (await isProjectSafeAutoRunEnabled(projectId)) {
    ...
    if (stopCondition) {
      if (stopCondition.shouldStop) {
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Safe autopilot stopped: ${stopCondition.reason}`,
        });
        return;
      }
      if (stopCondition.requiresApproval) {
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Safe autopilot paused: ${stopCondition.reason}. Waiting for approval to continue.`,
        });
        session.pendingApproval = true;
        return;
      }
    ...
    if (run.result === "waiting_human") {
      ...
      if (task?.autoApprovable && config.autoApproveSafeTasks) {
        ...
        publishProjectEvent({
          ...
          projectId,
          timestamp: new Date().toISOString(),
          message: `Safe autopilot auto-approved ${task.taskCode}. Completed tasks this session: ${session.tasksCompleted}.`,
        });
