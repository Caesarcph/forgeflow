...
export type WritebackDryRunResult = {
  dryRun: true;
  task: ReturnType<typeof serializeTask>;
  writebackPreview: DryRunResult;
};
...
export async function writebackTask(
  taskId: string,
  summary?: string,
  ...
  const writebackResult = await updateCheckboxInFile({
  ...
  return {
    dryRun: false,
    detail: await getProjectDetail(task.projectId),
  };
}
